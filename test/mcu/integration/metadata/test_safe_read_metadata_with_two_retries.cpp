...
public:
    void SetUp() override
    {
        ...
        data_[kCRCOffsetInMetadata + 1] = 0xAA;
    }
    static uint8_t onUartRead()
    {
        ...
        {
            pos_ = 0;
            if (retransmit_count_ != kMaxTriesWithCommunicationFailure - 1)
            {
                result = data_[pos_];
            }
            ++retransmit_count_;
...
uint8_t Fixture::retransmit_count_{};

TEST_F(Fixture, SafeReadMetadataFromHostTwoRetries_WhenTypical)
{
