#include <gtest/gtest.h>
#include <gmock/gmock.h>

#include <memory>
#include <string>

using ::testing::_;
using ::testing::InSequence;
using ::testing::Invoke;
using ::testing::Return;

#include "mcu/uartboot.h"

class UartBootMock : public UartBoot
{
public:
    MOCK_METHOD(uint8_t, uart_read, (), (const, override));
};

class Fixture : public ::testing::Test
{
public:
    static uint8_t onUartRead()
    {
        static uint8_t i{0};
        return data_[++i - 1];
    }

protected:
    UartBootMock sut_{};

private:
    static uint8_t data_[kPageWithMetadataSize];
};

uint8_t Fixture::data_[kPageWithMetadataSize]{};

TEST_F(Fixture, ReadPageFromHostOk_WhenTypical)
{
    uint8_t expected{kPageWithMetadataSize};
    uint8_t actual{0};
    uint8_t in[kPageWithMetadataSize];

    InSequence seq;
    for (uint8_t i = 0; i < kGlobalMetadataSize; ++i)
    {
        EXPECT_CALL(sut_, uart_read())
            .WillRepeatedly(Invoke(onUartRead))
            .RetiresOnSaturation();
    }

    actual = sut_.readPageWithMetadataFromHost(in);

    ASSERT_EQ(actual, expected);
}